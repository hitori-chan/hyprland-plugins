...
namespace NHyprnotify {
    ...
    namespace {
        ...
        std::unordered_map<std::string, SP<ITexture>> historyIcons; // source -> the 20px history-row mark
    ...
    // The history row's 20px mark: the entry keeps only a source string, so
    // the texture re-resolves at warm (the texture rule) and caches per source.
    // A vanished file settles to a null mark — the row text still identifies
    // the card. The cache is bounded by the history cap; Clear wipes it.
    SP<ITexture> historyIcon(const std::string& source, int iconPx) {
        if (source.empty())
            return nullptr;
        auto IT = historyIcons.find(source);
        if (IT != historyIcons.end()) {
            if (IT->second)
                return IT->second;
            historyIcons.erase(IT); // a rejected decode gets one retry per warm
        }
        const auto TEX = fileTex(source, iconPx);
        if (!TEX.settled)
            return nullptr; // the decode is pending; the next warm retries
        if (historyIcons.size() >= 64)
            historyIcons.clear(); // keep the map finite regardless of the history cap
        historyIcons.emplace(source, TEX.texture);
        return TEX.texture;
    }
