...
namespace NHyprnotify {
    ...
    double renderPopups(const SPaint& P, const SType& T, bool osdOnly, std::optional<double> startY, bool measureOnly) {
        ...
        const auto   COLFG = color(cfg.colFg), COLTITLE = color(cfg.colTitle), COLSUB = color(cfg.colKicker), COLURGENT = color(cfg.colUrgent), COLACC = color(cfg.colHighlight),
                     COLLINK     = color(cfg.colLink);
        ...
        for (const auto& N : notifs) {
            ...
            if (warmGate.warming) {
                ...
                ensureConversationIcons(*N, (int)std::lround(MAXICON * P.scale));
            }
            ...
            static std::vector<std::pair<const std::string*, const std::string*>> actionSrc; // reused; main thread only
            actionSrc.clear();
            if (N->canReply)
                actionSrc.emplace_back(&REPLY_ID, N->replyActionText.empty() ? &REPLY_LBL : &N->replyActionText);
            for (const auto& A : N->actions)
                actionSrc.emplace_back(&A.id, &A.label);
            static std::vector<CBox> btnBoxes; // reused; main thread only
            btnBoxes.clear();
            double btnH = 0;
            {
                double bx = 0, rowY = 0;
                for (size_t i = 0; i < actionSrc.size(); i++) {
                    const auto& [AID, ALABEL] = actionSrc[i];
                    auto& LB = scratch();
                    appendEsc(LB, *ALABEL);
                    const auto   LBL = cachedText(LB, COLACC, T.action, TEXTWPX, -1, 0, true, 600);
                    const double LW  = texW(LBL, P.scale);
                    const bool   IS_REPLY = AID == &REPLY_ID;
                    const size_t ACTION_INDEX = i - (N->canReply ? 1u : 0u);
                    const bool   HAS_ACTION_ICON = !IS_REPLY && N->actionIcons && ACTION_INDEX < N->actions.size() && N->actions[ACTION_INDEX].iconTex;
                    const double IW  = HAS_ACTION_ICON ? BTN_ICON + BTN_ICON_GAP : 0;
            ...
            const double AVAIL    = CARDH - 2 * PADY - HH - (HH > 0 ? HEAD_GAP : 0) - TH - TITLE_GAP - (N->progress >= 0 ? PROGRESS_GAP + PROGRESS_H : 0) - BTN_BLOCK - IMG_BLOCK;
            ...
            const double CH = std::min(CARDH, std::max(ICONW, th) + 2 * PADY);
            ...
            CP.glass(CARD, surface(), ROUND, RP);
            ...
            if (!btnBoxes.empty()) {
                ...
                for (size_t i = 0; i < btnBoxes.size(); i++) {
                    ...
                    if (BHOV)
                        CP.rect(BOX, stateLayer(), (int)std::lround(BTN_H / 2 * P.scale));
                    double cx = BOX.x + BTN_PADX;
                    if (AID != &REPLY_ID && N->actionIcons) {
                        const auto ACTION_INDEX = i - (N->canReply ? 1u : 0u);
                        if (ACTION_INDEX < N->actions.size() && N->actions[ACTION_INDEX].iconTex) {
                            CP.texFit(N->actions[ACTION_INDEX].iconTex, CBox{cx, BOX.y + (BOX.h - BTN_ICON) / 2, BTN_ICON, BTN_ICON}, 0);
