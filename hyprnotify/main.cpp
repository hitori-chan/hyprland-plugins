...
APICALL EXPORT PLUGIN_DESCRIPTION_INFO PLUGIN_INIT(HANDLE handle) {
    ...
    // v13 defaults: the color roles register at the INK set's values (the
    // v13() helpers treat "still at this default" as "follow the active
    // theme set"), so switching `theme` re-tunes every role the user never
    // overrode. Explicit user colors always win.
    namespace Th = NHyprCommon::Theme;
    cfg.theme           = makeShared<Config::Values::CStringValue>("plugin:hyprnotify:theme", "\"tray\" (the hyprbar tray-menu material, opaque — default), \"ink\" (the v13 AOSP palette) or \"glass\" (the frost): the v13 material set", "tray");
    ...
    cfg.offsetY         = makeShared<Config::Values::CIntValue>("plugin:hyprnotify:offset_y", "popups' and the center's distance from the monitor top", 26); // the 26px bar
    ...
    cfg.colBg           = makeShared<Config::Values::CColorValue>("plugin:hyprnotify:col_bg", "shade/panel glass container", (int)Th::INK.panel);
    cfg.colFg           = makeShared<Config::Values::CColorValue>("plugin:hyprnotify:col_fg", "on-surface body text", (int)Th::INK.on);
    cfg.colTitle        = makeShared<Config::Values::CColorValue>("plugin:hyprnotify:col_title", "strong on-surface titles", (int)Th::INK.on);
    cfg.colKicker       = makeShared<Config::Values::CColorValue>("plugin:hyprnotify:col_kicker", "on-surface-variant metadata", (int)Th::INK.on82);
    cfg.colFrame        = makeShared<Config::Values::CColorValue>("plugin:hyprnotify:col_frame", "outline/hairlines", (int)Th::INK.rim);
    cfg.colUrgent       = makeShared<Config::Values::CColorValue>("plugin:hyprnotify:col_urgent", "error role for critical content", Th::ERROR);
    cfg.colHighlight    = makeShared<Config::Values::CColorValue>("plugin:hyprnotify:col_highlight", "primary actions, progress, and focus", (int)Th::INK.action);
    cfg.colLink         = makeShared<Config::Values::CColorValue>("plugin:hyprnotify:col_link", "body hyperlinks", (int)Th::INK.action);
    cfg.colSurface      = makeShared<Config::Values::CColorValue>("plugin:hyprnotify:col_surface", "notification and row surface container", (int)Th::INK.card);
    cfg.colSurfaceHigh  = makeShared<Config::Values::CColorValue>("plugin:hyprnotify:col_surface_high", "raised control surface container", (int)Th::INK.raised);
    cfg.colState        = makeShared<Config::Values::CColorValue>("plugin:hyprnotify:col_state", "hover and selected state layer", (int)Th::INK.raisedH);
    cfg.colOnHighlight  = makeShared<Config::Values::CColorValue>("plugin:hyprnotify:col_on_highlight", "content on a primary fill", (int)Th::INK.onAccent);
    ...
    for (const auto& V : {cfg.theme, cfg.font, cfg.soundCommand})
        HyprlandAPI::addConfigValueV2(PHANDLE, V);
