...
use capture-space coordinates. The Reply and Mark as read labels share the
body's `x=123` content column; their hitboxes no longer depend on an optical
left pull. The footer remains `82 / 341 / 83px` with `12px` gaps and `56px`
buttons, while its 20dp ROM-derived history/settings vectors render at the
measured 28px capture size. On narrow
browsers the complete canvas scales as one unit instead of reflowing its
internal layout.
...
notification settings, history, the notification footer settings mark, and
the Do Not Disturb mark. `pixel-snooze.svg` is the exact ROM
`drawable/ic_snooze.xml` path, rendered at the measured `34px` action mark
size. OSD cards use the same popup padding, radius, surface, semantic icon
cell, and value-track anatomy as ordinary popup cards; only their semantic
identity and transient placement differ.
