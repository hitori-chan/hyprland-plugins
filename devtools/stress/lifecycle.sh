...
sleep 1
chk "input storm: all $NLOADED plugins alive" test "$(hq plugin list | grep -c Plugin)" = "$NLOADED"
...
chk "reload: all $NLOADED plugins alive" test "$(hq plugin list | grep -c Plugin)" = "$NLOADED"
